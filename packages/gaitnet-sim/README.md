...
| `cat` | constraints as terminations (`gaitnet_sim.env.constraints`): the limit-style penalties (`foot_slip`, `flat_orientation_l2`, `lin_vel_z_l2`, `ang_vel_xy_l2`) become thresholds in physical units (planted-foot slip 0.1 m/s, tilt 0.2 rad, vertical speed 0.3 m/s, roll/pitch rate 1.5 rad/s, heading drift 0.15 rad per second, stance 0.1 s before a leg lifts again), each ending the episode with probability up to `p_max` in proportion to how far it is broken; the fall penalty counts falls only, and neither the curriculum nor evaluation counts constraint terminations | — | ~0 |
